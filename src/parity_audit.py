...
def _reference_surface() -> dict[str, Any]:
    loaded = json.loads(REFERENCE_SURFACE_PATH.read_text())
...
def run_parity_audit() -> ParityAuditResult:
    ...
    return ParityAuditResult(
        ...
        total_file_ratio=(current_python_files, _to_int(reference.get("total_ts_like_files", 0))),
        command_entry_ratio=(_snapshot_count(COMMAND_SNAPSHOT_PATH), _to_int(reference.get("command_entry_count", 0))),
        tool_entry_ratio=(_snapshot_count(TOOL_SNAPSHOT_PATH), _to_int(reference.get("tool_entry_count", 0))),
