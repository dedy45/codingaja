...
def build_parser() -> argparse.ArgumentParser:
    ...
    subparsers.add_parser(
        "parity-audit", help="compare the Python workspace against the local ignored TypeScript archive when available"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "commands":
        if args.query:
            ...
            commands = get_commands(
                include_plugin_commands=not args.no_plugin_commands, include_skill_commands=not args.no_skill_commands
            )
    ...
    if args.command == "tools":
        if args.query:
            ...
            tools = get_tools(
                simple_mode=args.simple_mode, include_mcp=not args.no_mcp, permission_context=permission_context
            )
    ...
    if args.command == "turn-loop":
        results = PortRuntime().run_turn_loop(
            args.prompt, limit=args.limit, max_turns=args.max_turns, structured_output=args.structured_output
        )
